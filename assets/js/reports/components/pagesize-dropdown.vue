<template>
  <div class="dropdown-wrapper" @click="toggleMe">
    <b-dropdown
      :text="elementSelected ? elementSelected.txt : ''"
    ...
    >
      <template v-slot:button-content>
        <span class="black-button-text">{{
          elementSelected ? elementSelected.txt : ""
        }}</span>
      </template>
...
export default {
  ...
  methods: {
    selectedElement: function(val) {
      if (val === "delete") {
        ...
        this.$emit("changed-pagesize", { idx: this.index, val });
        this.elementSelected = this.elements.find(el => val == el.value);
      }
    },
    toggleMe: function() {
  ...
  beforeMount: function() {
    ...
    this.elementSelected = this.elements.find(
      el => this.defaultValue == el.value
    );
  },
  updated: function() {},
  created: function() {
