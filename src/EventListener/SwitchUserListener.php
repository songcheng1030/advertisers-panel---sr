...
class SwitchUserListener
{
    ...
    public function __construct(
        ...
        string $usernameParameter = '_switch_user',
        string $role = 'ROLE_ALLOWED_TO_SWITCH',
    ...
    private function attemptSwitchUser(Request $request, string $username): ?TokenInterface
    {
        ...
        if (false === $this->accessDecisionManager->decide($token, [$this->role], $user) ||
            !$originalUser->canImpersonateUser($user)
        ) {
        ...
        if ($request->hasSession() && ($session = $request->getSession()) && null !== $user && null !== $user->getLocale()) {
            $session->set(
                '_locale',
    ...
    private function attemptExitUser(Request $request): TokenInterface
    {
        if (null === ($currentToken = $this->tokenStorage->getToken()) ||
            null === $original = $this->getOriginalToken($currentToken)
        ) {
            throw new AuthenticationCredentialsNotFoundException('Could not find original Token object.');
        }
